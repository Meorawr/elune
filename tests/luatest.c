...
static void test_protecttaint_secure_normal (void) {
  ...
  TEST_CHECK((luaL_issecure(L)));  /* on success we should be secure */
  TEST_CHECK((luaL_issecurevalue(L, -1)));  /* the return value should also be secure */
  lua_close(L);
}
...
TEST_LIST = {
  { .name = "lua_protecttaint: stack remains secure after call", test_protecttaint_secure_normal },
  { .name = "lua_protecttaint: stack remains tainted after call", test_protecttaint_tainted_normal },
  { .name = "lua_protecttaint: stack restored to secure on error", test_protecttaint_secure_error },
  { .name = "lua_protecttaint: stack restored to tainted on error", test_protecttaint_tainted_error },
  { .name = "scripted test cases", test_scriptcases },
  { .name = NULL, .func = NULL },
};
