...
static void traceexec (lua_State *L, const Instruction *pc) {
    ...
    if (mask & LUA_MASKLINE) {
        ...
        if (npc == 0 || pc <= oldpc ||
            newline != getfuncline(p, pcRel(oldpc, p))) {
...
static int lessequal (lua_State *L, const TValue *l, const TValue *r) {
    ...
    if (ttype(l) != ttype(r)) {
        ...
        return l_strcmp(rawtsvalue(l), rawtsvalue(r)) <= 0;
    } else if ((res = call_orderTM(L, l, r, TM_LE)) !=
               -1) { /* first try `le' */
        return res;
    ...
    if ((b = luaV_tonumber(L, rb, &tempb)) != NULL &&
        ...
        switch (op) {
            case TM_ADD:
                ...
                break;
            case TM_SUB:
                setnvalue(L, ra, luai_numsub(nb, nc));
                break;
            case TM_MUL:
...
reentry: /* entry point */
    ...
    L->ts.vmexecmask =
        ((cl->taint != NULL) ? LUA_TAINTBLOCKED : LUA_TAINTALLOWED);
    ...
    for (;;) {
        ...
        if (((L->hookmask & LUA_MASKCOUNT) && (--L->hookcount == 0)) ||
            (L->hookmask & LUA_MASKLINE)) {
            luaG_profileleave(L);
            traceexec(L, pc);
        ...
        if (L->baseexeccount > 0 && (--L->execcount == 0)) {
            ...
            if (elapsed > L->baseexeclimit) {
                luaG_runerror(L, "script ran too long");
            }
        }
